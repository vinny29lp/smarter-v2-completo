...
const COL_MAP: Record<string, string> = {
  nome: "name", name: "name", "nome fantasia": "name", empresa: "name",
  "razao social": "razaoSocial", "razão social": "razaoSocial", razaosocial: "razaoSocial",
  cnpj: "cnpj",
  setor: "setor", segmento: "setor", ramo: "setor",
  email: "email", "e-mail": "email",
  telefone: "telefone", phone: "telefone", fone: "telefone",
  responsavel: "responsavel", "responsável": "responsavel", contato: "responsavel",
  cargo: "cargoResponsavel", "cargo do responsavel": "cargoResponsavel", "cargo do responsável": "cargoResponsavel",
...
function validateRow(row: Record<string, string>): { valido: boolean; motivo?: string } {
  ...
  if (!cnpjDigits || cnpjDigits.length !== 14) return { valido: false, motivo: "CNPJ inválido (14 dígitos)" };
  ...
  if (!row.cidade) return { valido: false, motivo: "Cidade obrigatória" };
  if (!row.uf) return { valido: false, motivo: "UF obrigatória" };
  return { valido: true };
}
...
export function ImportarEmpresasModal({ onClose, onSuccess }: { onClose: () => void; onSuccess?: () => void }) {
  const [step, setStep] = useState<Step>("upload");
  const [dragging, setDragging] = useState(false);
  const [fileName, setFileName] = useState("");
  const [empresas, setEmpresas] = useState<ParsedCompany[]>([]);
  const [parseError, setParseError] = useState("");
  const [result, setResult] = useState<ImportResult | null>(null);
  const [progress, setProgress] = useState(0);
  const fileRef = useRef<HTMLInputElement>(null);

  const validCount = empresas.filter(s => s._valido).length;
  const invalidCount = empresas.filter(s => !s._valido).length;
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const parsed: ParsedCompany[] = rows
        ...
        .map(r => {
          ...
          return {
            ...
            cargoResponsavel: mapped.cargoResponsavel,
  ...
  const confirmarImportacao = async () => {
    const validos = empresas.filter(s => s._valido);
    ...
    for (let i = 0; i < batches.length; i++) {
      const res = await fetch("/api/app/empresas/importar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ empresas: batches[i], arquivo: fileName }),
      });
  ...
  const downloadRelatorio = () => {
    ...
    a.href = url;
    a.download = `relatorio-importacao-empresas-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-lg font-black text-slate-800">📥 Importar Empresas</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              {step === "upload" && "Selecione um arquivo XLSX, XLS ou CSV"}
              {step === "preview" && `${empresas.length} registros encontrados — ${validCount} válidos, ${invalidCount} com erro`}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === "upload" && (
            <div>
              ...
              <div className="mt-4 p-4 bg-slate-50 rounded-xl">
                ...
                <p className="text-xs text-slate-400">Nome, Razão Social, CNPJ*, Setor, Email*, Telefone, Responsável, Cargo, Endereço, Bairro, Cidade*, UF*, CEP, Site</p>
                <p className="text-[10px] text-slate-400 mt-1">* obrigatórios</p>
              </div>
            </div>
          )}
            ...
            <div>
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="p-3 bg-blue-50 rounded-xl text-center">
                  <p className="text-2xl font-black text-blue-700">{empresas.length}</p>
              ...
              <p className="mt-2 text-xs text-slate-400 bg-slate-50 p-2 rounded-lg">
                ℹ Empresas migradas ficam visíveis para todas as unidades. Empresas cadastradas normalmente continuam privadas da unidade que cadastrou.
              </p>
            </div>
          )}
          ...
          {step === "importing" && (
            <div className="text-center py-8">
              ...
              <p className="font-bold text-slate-700 mb-3">Importando empresas...</p>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-between gap-3">
          ...
          {step === "preview" && (
            <>
              <button onClick={() => { setStep("upload"); setEmpresas([]); }} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-700">← Voltar</button>
              <button
