...
const COL_MAP: Record<string, string> = {
  nome: "name", name: "name", "nome completo": "name", "instituicao": "name", "instituição": "name", ies: "name",
  "razao social": "razaoSocial", "razão social": "razaoSocial", razaosocial: "razaoSocial",
  cnpj: "cnpj",
  tipo: "tipo", categoria: "tipo",
  email: "email", "e-mail": "email",
  telefone: "telefone", phone: "telefone", fone: "telefone",
  coordenador: "coordenador", "coordenador(a)": "coordenador", contato: "coordenador", responsavel: "coordenador", "responsável": "coordenador",
  cargo: "cargoCoord", "cargo do coordenador": "cargoCoord", "cargo coordenador": "cargoCoord",
  ...
  cursos: "cursos", "cursos oferecidos": "cursos",
};
...
function validateRow(row: Record<string, string>): { valido: boolean; motivo?: string } {
  if (!row.name) return { valido: false, motivo: "Nome obrigatório" };
  if (row.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(row.email)) return { valido: false, motivo: "E-mail inválido" };
  return { valido: true };
}
...
interface ImportResult {
  ...
  duplicados: number;
  erros: number;
  detalhes: { nome: string; status: "importado" | "duplicado" | "erro"; motivo?: string }[];
}

export function ImportarInstituicoesModal({ onClose, onSuccess }: { onClose: () => void; onSuccess?: () => void }) {
  const [step, setStep] = useState<Step>("upload");
  const [dragging, setDragging] = useState(false);
  const [fileName, setFileName] = useState("");
  const [instituicoes, setInstituicoes] = useState<ParsedInstitution[]>([]);
  const [parseError, setParseError] = useState("");
  const [result, setResult] = useState<ImportResult | null>(null);
  const [progress, setProgress] = useState(0);
  const fileRef = useRef<HTMLInputElement>(null);

  const validCount = instituicoes.filter(s => s._valido).length;
  const invalidCount = instituicoes.filter(s => !s._valido).length;
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const parsed: ParsedInstitution[] = rows
        ...
        .map(r => {
          ...
          return {
            ...
            coordenador: mapped.coordenador,
            cargoCoord: mapped.cargoCoord,
            cidade: mapped.cidade,
            uf: mapped.uf,
            endereco: mapped.endereco,
            cep: mapped.cep,
            site: mapped.site,
            cursos: mapped.cursos,
            _valido: valido,
            _motivo: motivo,
          };
        });
  ...
  const confirmarImportacao = async () => {
    const validos = instituicoes.filter(s => s._valido);
    ...
    const BATCH = 50;
    const batches: ParsedInstitution[][] = [];
    ...
    for (let i = 0; i < batches.length; i++) {
      const res = await fetch("/api/app/instituicoes/importar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ instituicoes: batches[i], arquivo: fileName }),
      });
  ...
  const downloadRelatorio = () => {
    ...
    const lines = [
      "Nome,Status,Motivo",
      ...result.detalhes.map(d => `"${d.nome}","${d.status}","${d.motivo || ""}"`),
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `relatorio-importacao-ies-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-lg font-black text-slate-800">📥 Importar Instituições (IES)</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              {step === "upload" && "Selecione um arquivo XLSX, XLS ou CSV"}
              {step === "preview" && `${instituicoes.length} registros encontrados — ${validCount} válidos, ${invalidCount} com erro`}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === "upload" && (
            <div>
              ...
              >
                ...
                <p className="text-sm text-slate-400 mt-1">Aceita: <strong>XLSX</strong>, <strong>XLS</strong>, <strong>CSV</strong> — até 500 registros</p>
              </div>
              ...
              <div className="mt-4 p-4 bg-slate-50 rounded-xl">
                ...
                <p className="text-xs text-slate-400">Nome, Razão Social, CNPJ, Tipo, Email, Telefone, Coordenador, Cargo, Cidade, UF, Endereço, CEP, Site, Cursos</p>
              </div>
            </div>
          )}
            ...
            <div>
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="p-3 bg-blue-50 rounded-xl text-center">
                  <p className="text-2xl font-black text-blue-700">{instituicoes.length}</p>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50 sticky top-0">
                      <tr>
                        ...
                        <th className="text-left px-3 py-2 font-bold text-slate-500">CNPJ</th>
                        <th className="text-left px-3 py-2 font-bold text-slate-500">Cidade/UF</th>
                        <th className="text-left px-3 py-2 font-bold text-slate-500">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {instituicoes.map((s, i) => (
                        <tr key={i} className={`border-t border-slate-50 ${s._valido ? "" : "bg-red-50/50"}`}>
                          <td className="px-3 py-2">{s.name || "—"}</td>
                          <td className="px-3 py-2 text-slate-500">{s.cnpj || "—"}</td>
                          <td className="px-3 py-2 text-slate-500">{s.cidade ? `${s.cidade}/${s.uf || ""}` : "—"}</td>
              ...
              <p className="mt-2 text-xs text-slate-400 bg-slate-50 p-2 rounded-lg">
                ℹ Instituições migradas ficam visíveis para todas as unidades, como já acontece hoje com as demais IES.
              </p>
            </div>
          )}
          ...
          {step === "importing" && (
            <div className="text-center py-8">
              ...
              <p className="font-bold text-slate-700 mb-3">Importando instituições...</p>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-between gap-3">
          ...
          {step === "preview" && (
            <>
              <button onClick={() => { setStep("upload"); setInstituicoes([]); }} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-700">← Voltar</button>
              <button
              ...
              >
                Importar {validCount} instituições →
              </button>
            </>
          )}
          {step === "result" && (
