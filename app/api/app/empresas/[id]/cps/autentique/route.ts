...
/** POST — envia CPS para assinatura via Autentique (1 e-mail) */
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Salvar authDocId e atualizar status
    await prisma.company.update({
      where: { id: params.id },
      data: {
        cpsStatus: "AGUARDANDO_ASSINATURA",
        cpsAuthDocId: resultado.id,
        cpsSignedUrl: null,
      } as any,
    });
    ...
    for (const signer of resultado.signatures || []) {
      if (signer.email && signer.link?.short_link) {
        enviarNotificacaoAssinatura({
          email: signer.email,
          nome: signer.name || empresa.responsavel || empresa.name,
          tipoDoc: `Contrato de Parceria — ${empresa.name}`,
...
/** GET — consulta status de assinatura do CPS no Autentique */
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!authDocId) {
      return NextResponse.json({ error: "CPS ainda não enviado para assinatura." }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      signedUrl: status.signedUrl || (empresa as any).cpsSignedUrl,
      signers: status.signers,
    });
  } catch (e: any) {
    return handleApiError(e, "EMPRESA_CPS_AUTENTIQUE_GET");
  }
}
