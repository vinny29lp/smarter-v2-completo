...
function header(titulo: string, subtitulo: string, numero: string, sm?: any): string {
  const logoHtml = sm?.logoDocUrl
    ? `<img src="${sm.logoDocUrl}" style="height:36px;object-fit:contain;" alt="logo"/>`
    : `<div style="background:#0f2a5e;color:#f5c400;font-weight:900;font-size:16px;width:36px;height:36px;border-radius:6px;display:flex;align-items:center;justify-content:center;">S</div>`;
  const nomeFantasia = sm?.razaoSocial ? sm.razaoSocial.split(" ").slice(0,2).join(" ") : "Smarter Estágios";
  return `<div class="header">
    <div style="display:flex;align-items:center;gap:8px">${logoHtml}<div style="font-size:9px;color:#666">${nomeFantasia}</div></div>
...
<div class="doc"><div class="watermark">${sm?.watermarkText || "SMARTER"}</div>
${header("TERMO DE COMPROMISSO DE ESTAGIO E ACORDO DE COOPERACAO","Conforme Lei No 11.788, de 25 de setembro de 2008",c.numero,c.smarter)}
...
<div class="pagebreak">
${header("PLANO DE ESTAGIO","Vinculado ao TCE No "+c.numero+" — Lei 11.788/2008",c.numero,c.smarter)}
...
export function gerarReciboBolsa(c: ContratoData, mesRef: string): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${DOC_CSS}</style></head><body>
<div class="doc">${header("RECIBO DE PAGAMENTO DE BOLSA-AUXILIO","Lei No 11.788/2008",c.numero,c.smarter)}
...
export function gerarRescisao(c: ContratoData, ultimoDia: string, motivo: string): string {
  ...
  const hoje = new Date().toLocaleDateString("pt-BR");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${DOC_CSS}</style></head><body>
<div class="doc">${header("RESCISAO AO TERMO DE COMPROMISSO DE ESTAGIO","Lei No 11.788/2008",c.numero,c.smarter)}
...
export function gerarReciboRescisao(c: ContratoData, diasBolsa: number, mesesRecesso: number, descontos: number): string {
  ...
  const hoje = new Date().toLocaleDateString("pt-BR");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${DOC_CSS}</style></head><body>
<div class="doc">${header("RECIBO DE RESCISAO","Lei No 11.788/2008",c.numero,c.smarter)}
...
export function gerarTermoRecesso(c: ContratoData, diasRecesso: number, dataIni: string, dataFim: string, periodo: string): string {
  ...
  const hoje = new Date().toLocaleDateString("pt-BR");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${DOC_CSS}</style></head><body>
<div class="doc">${header("TERMO DE RECESSO REMUNERADO","Art. 13 da Lei No 11.788/2008",c.numero,c.smarter)}
<div class="section" style="margin-top:16px"><div class="grid2">
${f("Estagiario(a)",e.nome,true)}
...
export function gerarTermoRealizacao(c: ContratoData, chTotal: number, desempenho: string): string {
  ...
  const hoje = new Date().toLocaleDateString("pt-BR");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${DOC_CSS}</style></head><body>
<div class="doc">${header("TERMO DE REALIZACAO DE ESTAGIO","Lei No 11.788/2008",c.numero,c.smarter)}
